...
int KSaveGame::saveload(int am_saving) {
    ...
    //TODO: Double check this sound is right
    Music.play_effect("whoosh");

    while (!stop) {
        //TODO Animations: Game.do_check_animation();
...
void KSaveGame::show_sgstats(int saving) {
    ...
    //TODO: Still need to figure out menubox colors and make it work right
    int DARKBLUE = 0, BLUE = 2;

    if (saving == 0) {
        Draw.menubox(0, pointer_offset + 12, 7, 1, BLUE);
        Draw.print_font(8, pointer_offset + 20, _("Loading"), FontColor::GOLD);
    } else if (saving == 1) {
        Draw.menubox(8, pointer_offset + 12, 6, 1, BLUE);
        Draw.print_font(16, pointer_offset + 20, _("Saving"), FontColor::GOLD);
    } else if (saving == 2 || saving == 3) {
        Draw.menubox(8, pointer_offset + 12, 6, 1, BLUE);
        Draw.print_font(16, pointer_offset + 20, _("Delete"), FontColor::RED);
    }
    ...
    if (top_pointer < NUMSG - max_onscreen) {
        dnptr->renderTo(32, KQ_SCREEN_H - 8);
    }
    ...
    for (sg = top_pointer; sg < top_pointer + max_onscreen; sg++) {
        ...
        if (sg == save_ptr) {
            Draw.menubox(72, pointer_offset, 29, 4, DARKBLUE);
        } else {
            Draw.menubox(72, pointer_offset, 29, 4, BLUE);
        }

        if (savegames[sg].num_characters == -1) {
            Draw.print_font(136, pointer_offset + 20, _("Wrong version"), FontColor::NORMAL);
        } else {
            if (stats.num_characters == 0) {
                Draw.print_font(168, pointer_offset + 20, _("Empty"), FontColor::NORMAL);
            } else {
                for (a = 0; a < stats.num_characters; a++) {
                    auto& chr = stats.characters[a];
                    hx = a * 72 + 84;
                    hy = pointer_offset + 12;
                    frames[chr.id][1]->renderTo(hx, hy + 4);
                    sprintf(strbuf, _("L: %02d"), chr.level);
                    Draw.print_font(hx + 16, hy, strbuf, FontColor::NORMAL);
                    Draw.print_font(hx + 16, hy + 8, _("H:"), FontColor::NORMAL);
                    Draw.print_font(hx + 16, hy + 16, _("M:"), FontColor::NORMAL);
                    Draw.rectfill(hx + 33, hy + 9, hx + 65, hy + 15, 2);
                    Draw.rectfill(hx + 33, hy + 9, hx + 65, hy + 15, 2);
                    Draw.rectfill(hx + 32, hy + 8, hx + 64, hy + 14, 35);
                    Draw.rectfill(hx + 33, hy + 17, hx + 65, hy + 23, 2);
                    Draw.rectfill(hx + 32, hy + 16, hx + 64, hy + 22, 19);
                    b = chr.hp * 32 / 100;
                    Draw.rectfill(hx + 32, hy + 9, hx + 32 + b, hy + 13, 41);
                    b = chr.mp * 32 / 100;
                    Draw.rectfill(hx + 32, hy + 17, hx + 32 + b, hy + 21, 25);
                }

                sprintf(strbuf, _("T %u:%02u"), stats.time / 60, stats.time % 60);
                Draw.print_font(236, pointer_offset + 12, strbuf, FontColor::NORMAL);
                sprintf(strbuf, _("G %u"), stats.gold);
                Draw.print_font(236, pointer_offset + 28, strbuf, FontColor::NORMAL);
            }
        }
    }
}
...
s_sgstats* KSaveGame::get_savegames(void) {
    return savegames;
}
